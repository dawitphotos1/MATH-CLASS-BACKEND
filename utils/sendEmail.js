...
// utils/sendEmail.js - BASIC WORKING VERSION
import nodemailer from 'nodemailer';
import dotenv from 'dotenv';
...
const transporter = nodemailer.createTransport({
  ...
  // For Render/Gmail compatibility
  tls: {
...
transporter.verify((error, success) => {
  if (error) {
    ...
    console.log('   User:', process.env.MAIL_USER ? `${process.env.MAIL_USER.substring(0, 3)}...` : 'Not set');
...
export const sendEmail = async ({ to, subject, html }) => {
  try {
    ...
    if (error.code === 'EAUTH') {
      ...
      console.error('❌ Connection failed. Check MAIL_HOST and MAIL_PORT.');
    }
