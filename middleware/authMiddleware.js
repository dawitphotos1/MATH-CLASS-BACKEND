...
/**
 * This file contains ONLY role-check middleware.
 * authenticateToken.js handles JWT verification and attaches req.user.
 *
 * These functions require `authenticateToken` to be used BEFORE them.
 * Example:
 *   router.get("/admin", authenticateToken, isAdmin, controller);
 */
...
/* ============================================================
   Teacher or Admin (combined helper)
============================================================ */
export const isTeacherOrAdmin = (req, res, next) => {
  if (!req.user) {
    return res.status(401).json({ success: false, error: "Authentication required" });
