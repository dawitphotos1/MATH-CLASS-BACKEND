...
// ================================
// URL FIXING FUNCTION - IMPROVED
// ================================

const fixCloudinaryUrl = (url) => {
  if (!url || typeof url !== "string") return url;
  
  // Fix Cloudinary URL if it's incorrectly using /image/upload/ for documents
...
const uploadToCloudinary = (
  ...
  return new Promise((resolve, reject) => {
    ...
    const isDocument = filename.toLowerCase().match(/\.(pdf|doc|docx|ppt|pptx|xls|xlsx|txt)$/);
    const isVideo = filename.toLowerCase().match(/\.(mp4|mov|avi|webm|wmv)$/);
    const isImage = filename.toLowerCase().match(/\.(jpg|jpeg|png|gif|webp)$/);

    if (isDocument || folder.includes("/pdfs/") || options.resource_type === "raw") {
      finalResourceType = "raw";
      finalFolder = "mathe-class/documents";
    } else if (isVideo) {
