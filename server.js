...
import pdfProxyRouter from "./routes/pdfProxy.js";
...
app.use("/api/v1/pdf-proxy", pdfProxyRouter);
...
app.get("/api/v1/health", async (_, res) => {
  try {
    ...
    res.json({
      ...
      status: "healthy",
      database: "connected",
      email: process.env.EMAIL_ENABLED !== "false" ? "configured" : "disabled",
...
(async () => {
  try {
    ...
    console.log("   - Note: Email transporter will lazy-load on first use");
    ...
    app.listen(PORT, "0.0.0.0", () => {
      ...
      console.log(`   - /api/v1/email/contact`);
      console.log(`   - /api/v1/email/status`);
    });
  } catch (err) {
