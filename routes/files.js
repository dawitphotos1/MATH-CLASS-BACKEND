...
/* Debug - list a few files and existence */
router.get("/debug", (req, res) => {
  try {
    const files = fs.existsSync(uploadsDir) ? fs.readdirSync(uploadsDir).slice(0, 50) : [];
    res.json({ success: true, uploadsDir, count: files.length, sample: files.slice(0, 10) });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});

/* GET file by filename (safe) - maps to /api/v1/files/:filename */
router.get("/:filename", (req, res) => {
  try {
    ...
    // Use res.sendFile which handles streaming + headers nicely
