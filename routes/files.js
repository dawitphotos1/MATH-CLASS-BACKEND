...
/**
 * ✅ FIXED: Serve uploaded files publicly (for PDFs, images, etc.)
 * GET /api/v1/files/uploads/:filename
 */
router.get("/uploads/:filename", async (req, res) => {
  try {
    ...
    // ✅ FIXED: Security - prevent directory traversal attacks
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    // ✅ FIXED: Create safe filename with timestamp and original name
...
const upload = multer({
  storage,
  limits: { 
    fileSize: 100 * 1024 * 1024 // ✅ INCREASED: 100MB limit for videos
  },
  fileFilter: (req, file, cb) => {
    const allowedTypes = [
      "image/jpeg",
  ...
  async (req, res) => {
    try {
      ...
      const fileInfo = {
        ...
        originalName: req.file.originalname,
        path: `/Uploads/${req.file.filename}`,
        fullUrl: `${process.env.BACKEND_URL || 'http://localhost:3000'}/Uploads/${req.file.filename}`,
        size: req.file.size,
        sizeFormatted: formatFileSize(req.file.size),
        type: path.extname(req.file.originalname).toLowerCase(),
        mimetype: req.file.mimetype,
  ...
  async (req, res) => {
    try {
      ...
      const uploadedFiles = req.files.map(file => ({
        ...
        fullUrl: `${process.env.BACKEND_URL || 'http://localhost:3000'}/Uploads/${file.filename}`,
        size: file.size,
