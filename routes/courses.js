...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.log(`📚 Fetching full course structure for course ${courseId}, teacher ${teacherId}`);
          ...
          if (unit.lessons) {
            unit.lessons.forEach(lesson => {
              ...
              if (lesson.video_url && !lesson.video_url.startsWith('http')) {
                lesson.video_url = `${process.env.BACKEND_URL || 'http://localhost:3000'}/api/v1/files${lesson.video_url}`;
              }
              // Build file URL
              if (lesson.file_url && !lesson.file_url.startsWith('http')) {
                lesson.file_url = `${process.env.BACKEND_URL || 'http://localhost:3000'}/api/v1/files${lesson.file_url}`;
              }
            });
          }
        });
      ...
      console.log(`✅ Successfully fetched full course structure for ${courseData.title} with ${courseData.units?.length || 0} units`);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.log(`✅ Successfully fetched full structure for ${coursesWithUrls.length} courses`);
  ...
  try {
    ...
    const lessonsWithUrls = lessons.map(lesson => {
      ...
      if (lessonData.video_url && !lessonData.video_url.startsWith('http')) {
        lessonData.video_url = `${process.env.BACKEND_URL || 'http://localhost:3000'}/api/v1/files${lessonData.video_url}`;
      }
      
      // Build file URL
      if (lessonData.file_url && !lessonData.file_url.startsWith('http')) {
        lessonData.file_url = `${process.env.BACKEND_URL || 'http://localhost:3000'}/api/v1/files${lessonData.file_url}`;
      }
