...
export const getPreviewLessonForCourse = async (req, res) => {
  try {
    ...
    console.log(`📊 Found preview lesson: ${lesson ? `Yes (ID: ${lesson.id})` : 'No'}`);

    // If no lesson is marked as preview, find the first lesson with content
    if (!lesson) {
      console.log("🔄 No lesson marked as preview, searching for first lesson with content...");
      ...
      // If still no lesson with content, just take the first lesson
      if (!lesson) {
        lesson = allLessons[0];
        console.log(`⚠️ No lesson with content found, using first lesson: ${lesson.title}`);
      } else {
        console.log(`✅ Found lesson with content: ${lesson.title}`);
      }
    }
...
export const checkCoursePreviewStatus = async (req, res) => {
  try {
    ...
    const lessonsWithFiles = allLessons.filter(l => 
      l.file_url || 
      (l.attachments && l.attachments.length > 0)
    );
    ...
    res.json({
      ...
      lessonsWithFiles: lessonsWithFiles.length,
...
export const markLessonAsPreview = async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      error: "Failed to mark lesson as preview" 
    });
  }
};
...
export const getLessonsByCourse = async (req, res) => {
  try {
    ...
    const lessonsWithUrls = lessons.map((lesson) => {
      const lessonData = buildLessonUrls(lesson);
      return lessonData;
    });
