...
export const getPreviewLessonForCourse = async (req, res) => {
  try {
    console.log(`🔍 Looking for preview lesson for course: ${req.params.courseId}`);
    
    // First, try to find a lesson explicitly marked as preview
    ...
    if (!lesson) {
      console.log("⚠️ No lesson with is_preview: true, looking for first lesson");
      // Get all lessons to see what's available
      ...
      // Try to find a lesson with a file
      lesson = allLessons.find(l => l.file_url) || allLessons[0];
      console.log(`📋 Using lesson ${lesson.id} as fallback preview`);
    } else {
      console.log(`✅ Found preview lesson: ${lesson.id} - "${lesson.title}"`);
    }
    
    const course = await Course.findByPk(req.params.courseId, {
      attributes: ["id", "title", "slug", "thumbnail", "teacher_id", "price"],
    });
    ...
    // Build URLs with Cloudinary fixes
    const previewLesson = buildFileUrls(lesson);
    ...
    res.json({
      ...
      message: lesson.is_preview 
        ? "Preview lesson found" 
        : "First lesson used as preview",
    });
  } catch (err) {
    console.error("❌ Preview lesson error:", err);
    res.status(500).json({
...
export const markLessonAsPreview = async (req, res) => {
  try {
    ...
    // First, unmark all other preview lessons in this course
    if (isPreview) {
      await Lesson.update(
        { is_preview: false },
        { 
          where: { 
            ...
            id: { [db.Sequelize.Op.ne]: lessonId }
          }
        }
      );
    }
    ...
    // Update this lesson
    await lesson.update({ is_preview: isPreview });
    
    res.json({
      success: true,
      message: `Lesson ${isPreview ? 'marked as' : 'unmarked from'} preview`,
      lesson: buildFileUrls(lesson),
    });
  } catch (err) {
      ...
      include: [
        {
          model: Lesson,
          as: "lessons",
          attributes: ["id", "title", "is_preview", "file_url", "content_type"],
          order: [["order_index", "ASC"]],
        },
      ],
    });
    ...
    res.json({
      ...
      previewStatus: {
        ...
        previewLesson: previewLesson ? {
          ...
          fileUrl: previewLesson.file_url ? previewLesson.file_url.substring(0, 100) + '...' : null,
        } : null,
        ...
        recommendedAction: !previewLesson && firstLesson 
          ? `Run: PUT /api/v1/lessons/${firstLesson.id}/mark-preview with {"isPreview": true}`
