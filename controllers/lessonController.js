...
const buildFileUrls = (lesson) => {
  ...
  // ✅ FIXED: Proper file URL generation with path cleaning
  if (lessonData.file_url && lessonData.file_url !== null && lessonData.file_url !== "" && !lessonData.file_url.startsWith('http')) {
    let cleanFileUrl = lessonData.file_url;
    // Remove any leading slashes that might cause issues
    if (cleanFileUrl.startsWith('/')) {
      cleanFileUrl = cleanFileUrl.substring(1);
    }
    
    const fullFileUrl = `${process.env.BACKEND_URL || 'http://localhost:3000'}/api/v1/files/${cleanFileUrl}`;
    console.log("📄 File URL transformed:", {
      original: lessonData.file_url,
      clean: cleanFileUrl,
      final: fullFileUrl
    });
    lessonData.file_url = fullFileUrl;
  ...
  if (lessonData.video_url && lessonData.video_url !== null && lessonData.video_url !== "" && !lessonData.video_url.startsWith('http')) {
    let cleanVideoUrl = lessonData.video_url;
    if (cleanVideoUrl.startsWith('/')) {
      cleanVideoUrl = cleanVideoUrl.substring(1);
    }
    
    const fullVideoUrl = `${process.env.BACKEND_URL || 'http://localhost:3000'}/api/v1/files/${cleanVideoUrl}`;
    console.log("🎥 Video URL transformed:", {
      original: lessonData.video_url,
      clean: cleanVideoUrl,
      final: fullVideoUrl
    });
    lessonData.video_url = fullVideoUrl;
