...
/* Normalize DB-stored paths (remove leading Uploads/ or slashes) */
const normalizePath = (p = "") => p.replace(/^Uploads\//i, "").replace(/^\/+/, "");

/* Build absolute file URLs for a lesson record */
const buildFileUrls = (lesson) => {
  if (!lesson) return null;
  const data = lesson.toJSON ? lesson.toJSON() : { ...lesson };
  const backend = getBackendUrl();
  ...
  if (data.video_url && !data.video_url.startsWith("http")) {
    data.video_url = `${backend}/api/v1/files/${normalizePath(data.video_url)}`;
  }
  if (data.file_url && !data.file_url.startsWith("http")) {
    data.file_url = `${backend}/api/v1/files/${normalizePath(data.file_url)}`;
  }
...
/* ------------------------ Public: get preview lesson for a course ------------------------ */
/**
 * Returns the lesson marked `is_preview: true` for a course.
 * If none found, falls back to the first lesson (if exists) so frontend can still preview.
 */
export const getCoursePreviewLesson = async (req, res) => {
  try {
    const { courseId } = req.params;
    ...
    // try to find dedicated preview lesson
    let lesson = await Lesson.findOne({
      where: { course_id: courseId, is_preview: true },
      order: [["order_index", "ASC"]],
      include: [{ model: Course, as: "course", attributes: ["id", "title", "slug", "teacher_id"] }],
    });
    ...
    if (!lesson) {
      return res.status(404).json({ success: false, error: "No lessons available for this course" });
    }
    ...
    return res.json({
      ...
      access: "public",
      fallback,
      message: fallback ? "Using first lesson as preview (no dedicated preview lesson set)" : "Preview lesson found",
    });
  } catch (err) {
    console.error("getCoursePreviewLesson error:", err);
    return res.status(500).json({ success: false, error: "Failed to load preview lesson" });
  }
};

/* ------------------------ Create / Update / Get handlers (unchanged core behavior) ------------------------ */

export const createLesson = async (req, res) => {
  try {
    const { courseId } = req.params;
    const { title, content, contentType, unitId, orderIndex, isPreview, videoUrl } = req.body;
    ...
    const full = await Lesson.findByPk(lesson.id, { include: [{ model: Course, as: "course" }, { model: Unit, as: "unit" }] });
    return res.status(201).json({ success: true, lesson: buildFileUrls(full) });
  } catch (err) {
    console.error("createLesson error:", err);
    return res.status(500).json({ success: false, error: err.message });
...
export const updateLesson = async (req, res) => {
  try {
    ...
    await existing.update(updates);
    const updated = await Lesson.findByPk(lessonId, { include: [{ model: Course, as: "course" }, { model: Unit, as: "unit" }] });
    return res.json({ success: true, lesson: buildFileUrls(updated) });
  } catch (err) {
    console.error("updateLesson error:", err);
    return res.status(500).json({ success: false, error: err.message });
...
export const getLessonById = async (req, res) => {
  try {
    const { lessonId } = req.params;
    const lesson = await Lesson.findByPk(lessonId, { include: [{ model: Course, as: "course" }, { model: Unit, as: "unit" }] });
    if (!lesson) return res.status(404).json({ success: false, error: "Lesson not found" });
    trackLessonView(req.user?.id, lessonId).catch(() => {});
    return res.json({ success: true, lesson: buildFileUrls(lesson) });
  } catch (err) {
    console.error("getLessonById error:", err);
    return res.status(500).json({ success: false, error: err.message });
...
export const getLessonsByCourse = async (req, res) => {
  try {
    const lessons = await Lesson.findAll({ where: { course_id: req.params.courseId }, order: [["order_index", "ASC"]], include: [{ model: Unit, as: "unit" }] });
...
export const getLessonsByUnit = async (req, res) => {
  try {
    const lessons = await Lesson.findAll({ where: { unit_id: req.params.unitId }, order: [["order_index", "ASC"]] });
    return res.json({ success: true, lessons: lessons.map(buildFileUrls) });
  } catch (err) {
    console.error("getLessonsByUnit error:", err);
    return res.status(500).json({ success: false, error: err.message });
