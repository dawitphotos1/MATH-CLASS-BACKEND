...
const trackLessonView = async (userId, lessonId) => {
  try {
    ...
    console.warn("trackLessonView failed:", err.message);
  }
};

const handleFileUploads = (req) => {
  const out = {};
  if (req.files?.video?.[0]) out.video_url = `Uploads/${req.files.video[0].filename}`;
  if (req.files?.pdf?.[0]) out.file_url = `Uploads/${req.files.pdf[0].filename}`;
  if (req.files?.file?.[0]) out.file_url = `Uploads/${req.files.file[0].filename}`;
  return out;
};
...
export const createLesson = async (req, res) => {
  try {
    ...
    const uploads = handleFileUploads(req);
    let finalType = uploads.file_url ? "pdf" : uploads.video_url ? "video" : contentType || "text";
    let file_path = uploads.file_url || null;
    let video_path = uploads.video_url || videoUrl || null;
