...
// ✅ Confirm Payment — now sets approval_status = "pending"
export const confirmPayment = async (req, res) => {
  try {
    const { sessionId, session_id, courseId, course_id } = req.body;
    const sid = sessionId || session_id;
    ...
    // ✅ Mark payment as paid but still pending approval
    const [access] = await UserCourseAccess.findOrCreate({
      where: { user_id: userId, course_id: cid },
      defaults: {
        payment_status: "paid",
    ...
    try {
      await sendEmail(
        ...
        `<p>Thank you for your payment for <strong>${course.title}</strong>.</p>
         <p>Your enrollment is now awaiting admin approval. You’ll receive an email once approved.</p>`
      );
    } catch (err) {
      console.warn("⚠️ Email send failed:", err.message);
    }
