...
/* ============================================================
   👩‍🎓 STUDENT MANAGEMENT (based on ENROLLMENTS)
============================================================ */

/**
 * Get all students grouped by enrollment approval status
 * @route GET /admin/students?status=pending|approved|rejected
 */
export const getStudentsByStatus = async (req, res) => {
  try {
    const { status } = req.query;
    console.log("🔍 ADMIN: Fetching students with enrollment status:", status);
    ...
    return res.json({
      ...
      students: enrollments, // frontend will map this
    });
  } catch (err) {
    console.error("❌ Error fetching students:", err);
...
export const getEnrollmentsByStatus = async (req, res) => {
  try {
    const { status } = req.query;
    console.log("🎯 ADMIN: Fetching enrollments:", status || "all");
...
export const approveEnrollment = async (req, res) => {
  try {
    ...
    // Optional: update UserCourseAccess if present
    if (UserCourseAccess) {
      await UserCourseAccess.update(
        { approval_status: "approved" },
        { where: { user_id: enrollment.user_id, course_id: enrollment.course_id } }
...
/**
 * Debug enrollments (for development)
 * @route GET /admin/debug/enrollments
 */
export const debugEnrollments = async (req, res) => {
  try {
    ...
    console.log("📊 DEBUG Enrollment Stats:", stats);
    res.json({ success: true, summary: stats, enrollments: all });
  } catch (err) {
    console.error("❌ Debug enrollments error:", err);
