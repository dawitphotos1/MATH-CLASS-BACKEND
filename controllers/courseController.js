...
const { Course, Lesson, User, Unit, Enrollment, Attachment } = db;
...
/* ============================================================
   Get public course by slug (with lessons) - FIXED VERSION
============================================================ */
export const getPublicCourseBySlug = async (req, res) => {
  try {
    const { slug } = req.params;
    
    console.log(`🔍 Getting public course by slug: ${slug}`);
    ...
    // Import buildLessonUrls from lessonController
    const { buildLessonUrls } = await import("./lessonController.js");

    // Build URLs for lessons using the proper function from lessonController
    if (courseData.lessons && Array.isArray(courseData.lessons)) {
      courseData.lessons = courseData.lessons.map((lesson) => {
        return buildLessonUrls(lesson);
      });
    }

    // Find preview lesson
    const previewLesson = courseData.lessons?.find(lesson => lesson.isPreview) || 
                         (courseData.lessons?.length > 0 ? courseData.lessons[0] : null);
...
/* ============================================================
   Get lessons for a specific course - FIXED VERSION
============================================================ */
...
/* ============================================================
   Get course with full structure (units and lessons) - FIXED
============================================================ */
export const getCourseWithStructure = async (req, res) => {
  try {
    ...
    console.log(`📥 Getting course structure: courseId=${courseId}, teacherId=${teacherId}`);
    ...
    const course = await Course.findOne({
      where: { 
        ...
        teacher_id: teacherId // Ensure teacher owns the course
      },
      attributes: ["id", "title", "description", "slug", "price", "thumbnail", "teacher_id", "created_at"],
      include: [
        {
          model: Unit,
          as: "units",
    ...
    if (courseData.thumbnail && !courseData.thumbnail.startsWith("http")) {
      courseData.thumbnail = `${backend}/api/v1/files/${encodeURIComponent(courseData.thumbnail)}`;
    }
    
    // Import buildLessonUrls from lessonController
    const { buildLessonUrls } = await import("./lessonController.js");
    
    // Fix lesson file URLs using buildLessonUrls
    if (courseData.units) {
      courseData.units.forEach(unit => {
        if (unit.lessons) {
          unit.lessons.forEach(lesson => {
            // Replace the lesson with properly built URLs
            Object.assign(lesson, buildLessonUrls(lesson));
          });
        }
      });
    }
...
export const getTeacherCourseFull = async (req, res) => {
  try {
    ...
    console.log(`📥 GET /teacher/${courseId}/full for teacher ${teacherId}`);
  ...
  try {
    ...
    res.json({
      ...
      lesson: {
        id: lesson.id,
        title: lesson.title,
        original_file_url: lesson.file_url,
        original_video_url: lesson.video_url,
        attachments_count: lesson.attachments?.length || 0,
      },
      builtUrls: {
        fileUrls: builtUrls.fileUrls,
        videoUrls: builtUrls.videoUrls,
        attachments: builtUrls.attachments,
      },
      analysis: {
        hasFileUrls: builtUrls.fileUrls.length > 0,
        hasVideoUrls: builtUrls.videoUrls.length > 0,
        hasAttachments: builtUrls.attachments.length > 0,
        firstFileUrl: builtUrls.fileUrls[0] || null,
        isCloudinary: builtUrls.fileUrls[0]?.includes('cloudinary.com') || false,
        usesRawUpload: builtUrls.fileUrls[0]?.includes('/raw/upload/') || false,
      },
    });
  } catch (error) {
...
export const debugCoursePreview = async (req, res) => {
  try {
    ...
    console.log(`🔍 Debugging preview for course: ${courseId}`);
...
/* ============================================================
   NEW: Enhanced debug preview endpoint for frontend
============================================================ */
export const debugEnhancedPreview = async (req, res) => {
  try {
    const { courseId } = req.params;
    
    console.log(`🔍 DEBUG ENHANCED: Getting preview for course ID: ${courseId}`);
    ...
    // Find preview lesson
    let previewLesson = course.lessons.find(l => l.is_preview);
    if (!previewLesson && course.lessons.length > 0) {
      previewLesson = course.lessons[0];
    }
    ...
    const builtLesson = buildLessonUrls(previewLesson);
    ...
    const debugInfo = {
      ...
      isPreviewFlag: previewLesson.is_preview,
      contentType: previewLesson.content_type,
      hasContent: !!previewLesson.content,
      contentLength: previewLesson.content?.length || 0,
      hasFileUrls: builtLesson.fileUrls?.length > 0,
      fileUrls: builtLesson.fileUrls,
      hasVideoUrls: builtLesson.videoUrls?.length > 0,
      videoUrls: builtLesson.videoUrls,
      hasAttachments: builtLesson.attachments?.length > 0,
      attachments: builtLesson.attachments,
      rawFileUrl: previewLesson.file_url,
      rawContent: previewLesson.content?.substring(0, 200) + (previewLesson.content?.length > 200 ? "..." : ""),
    };
