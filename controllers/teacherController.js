...
//           title: course.title,
//           units: course.units?.length || 0,
//           lessons: course.units?.reduce((sum, unit) => sum + (unit.lessons?.length || 0), 0) || 0,
//         },
...
export const getTeacherCourseFull = async (req, res) => {
  try {
    ...
    console.error("❌ Course full structure error:", err);
    return res.status(500).json({ error: "Failed to load course structure" });
  }
};
...
export const getAllTeacherCoursesFull = async (req, res) => {
  try {
    ...
    console.error("❌ Teacher courses full error:", err);
    return res.status(500).json({ error: "Failed to load teacher course list" });
  }
};
