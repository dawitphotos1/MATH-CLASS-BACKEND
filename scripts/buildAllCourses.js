...
// const normalizeLessonData = (lesson) => {
...
//         // Create unit header as a lesson
//         const unitHeaderLesson = await Lesson.create({
//           course_id: course.id,
//           unit_id: unit.id,
//           title: unitTitle,
//           order_index: (unitIndex * 100) + 0,
//           content_type: "unit_header",
//           content: `# ${unitTitle}\n\nThis unit covers important concepts and skills.`,
//           is_preview: false,
//         });
//         totalLessons++;
...
//           const normalizedLesson = normalizeLessonData(lessonItem);
          
//           // Validate lesson title
//           if (!normalizedLesson.title || normalizedLesson.title.trim() === '') {
//             console.warn(`⚠️ Skipping lesson with empty title in unit: ${unitTitle}`);
//             continue;
//           }
...
//             course_id: course.id,
//             unit_id: unit.id,
//             title: normalizedLesson.title.trim(),
//             order_index: (unitIndex * 100) + lessonIndex + 1,
//             content_type: normalizedLesson.content_type || "text",
//             content: `# ${normalizedLesson.title}\n\n## Learning Objectives\n- Understand key concepts\n- Apply knowledge to solve problems\n- Develop critical thinking skills\n\n## Lesson Content\nThis lesson covers important concepts and includes practice exercises.\n\n### Key Points\n- Point 1\n- Point 2\n- Point 3\n\n### Practice Exercises\n1. Exercise 1\n2. Exercise 2\n3. Exercise 3`,
//             video_url: normalizedLesson.content_type === "video" ? '/Uploads/sample-video.mp4' : null,
//             file_url: normalizedLesson.content_type === "pdf" ? '/Uploads/sample-lesson.pdf' : null,
//             is_preview: lessonIndex < 2,
//           });
//           totalLessons++;
//         }
...
//     console.log("\n📁 Creating sample files in Uploads directory...");
//     const sampleFiles = [
//       'default-course.jpg',
//       'sample-video.mp4', 
//       'sample-lesson.pdf'
...
//         fs.writeFileSync(filePath, `Sample content for ${filename}`);
...
// Complete curriculum data for all 6 courses
const allCoursesData = [
  {
    title: "Algebra 1",
    slug: "algebra-1",
...
const buildAllCourses = async () => {
  try {
    ...
    for (const courseData of allCoursesData) {
      ...
      for (let unitIndex = 0; unitIndex < courseData.curriculum.length; unitIndex++) {
        ...
        for (let lessonIndex = 0; lessonIndex < lessonItems.length; lessonIndex++) {
          ...
          // ✅ CRITICAL FIX: Set both course_id AND unit_id
          await Lesson.create({
            course_id: course.id,
            unit_id: unit.id, // ✅ THIS WAS MISSING - now properly set!
            title: normalizedLesson.title.trim(),
            order_index: (unitIndex * 100) + lessonIndex + 1,
            content_type: normalizedLesson.content_type || "text",
            content: `# ${normalizedLesson.title}\n\n## Learning Objectives\n- Understand key concepts\n- Apply knowledge to solve problems\n- Develop critical thinking skills\n\n## Lesson Content\nThis lesson covers important concepts and includes practice exercises.\n\n### Key Points\n- Point 1\n- Point 2\n- Point 3\n\n### Practice Exercises\n1. Exercise 1\n2. Exercise 2\n3. Exercise 3`,
    ...
    verifyCourses.forEach(course => {
      const courseLessonCount = course.units.reduce((total, unit) => total + unit.lessons.length, 0);
      verifiedLessonCount += courseLessonCount;
      console.log(`   ✅ ${course.title}: ${course.units.length} units, ${courseLessonCount} lessons`);
    });

    console.log(`\n✅ VERIFICATION COMPLETE:`);
    console.log(`   Total courses: ${verifyCourses.length}`);
    console.log(`   Total lessons verified: ${verifiedLessonCount}`);
